...
EGLBoolean proxy_eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    eglQuerySurface(dpy, surface, EGL_WIDTH, &menu.width);
    ...
    if (!menu.imgui_initialized) {
        SetupImGui();
        eglSwapInterval(dpy, 1); // vsync
        menu.imgui_initialized = true;
    }

    ImGuiIO &io = ImGui::GetIO();
    ImGuiStyle& style = ImGui::GetStyle();

    io.DisplaySize = ImVec2((float)menu.width, (float)menu.height);
    io.MousePos = ImVec2(input.x * scale_x, input.y * scale_y);
    io.MouseDown[0] = input.down;
